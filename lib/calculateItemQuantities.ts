...
export function calculateItemQuantitiesFromMeasurements(
  ...
  priceItems.forEach((item) => {
    ...
    if (coverage && coverageUnit) {
      ...
      // Starter is linear — quantity is based on eave + rake length.
      // If the item has coverage, divide by it to get bundles/units.
      const linearFeet = (m.eave_length || 0) + (m.rake_length || 0);
      if (coverage && coverage > 0) {
        qty = Math.ceil(linearFeet / coverage);
      } else {
        qty = linearFeet;
      }
    } else if (
